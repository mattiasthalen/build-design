...
| `read the change against the brief` | asked for, and the gap it closes is the one the other two reviews cannot: `/code-review` asks whether the code is right and `/security-review` asks whether it is safe, and a change can pass both while building something the brief did not ask for or missing a criterion it did. The brief is what the design agreed to, so it is the only one of the three that can say the work is done — both ways: every criterion met by a test, and nothing in the diff the brief never asked for |
...
| `That report ends the run: one ticket, and the next one is a new session` | asked for. Nothing else stopped it: the reading names the other open questions, the peers are warm, the plan mode ceremony is spent, and a model that has just reported a finished ticket will offer the next one as the obvious next thing. The context that made this ticket good is what makes the next one worse — the reading is this ticket's, the peers were asked this ticket's question, and the build filled the window it was supposed to keep slim. A fresh session is the skill's premise, and this is the line that holds it |
...
| `the brief's acceptance criteria` | the line said "its acceptance criteria where the ticket has none", on `design-interview`'s claim that criteria written from the design prove the design is done — and it read the ticket as the only place they could live. The brief always carries them. Written the old way the reading invents what it could have found, and the invention is plausible enough that nobody catches it: `source` on each criterion and the `unsourced` value are what make the difference visible |
| `one you wrote yourself is a sign you have not found the brief` | the positive form of the same rule. A missing brief is a real state — a ticket that never came from an interview — and the reading's job there is to say so, not to fill the hole and let step 4 build against criteria the design never agreed to |
